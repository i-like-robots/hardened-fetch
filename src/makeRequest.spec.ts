...
describe('Make request', () => {
  ...
  describe('Good responses', async () => {
    it('resolves with the HTTP response', async () => {
      ...
      const response = await makeRequest({ url: 'http://example.com/ok', retries: 0 })
  ...
  describe('Bad responses', () => {
    it('rejects with a 404 errors', async () => {
      ...
      await assert.rejects(
        async () => makeRequest({ url: 'http://example.com/404', retries: 0 }),
        'NotFound'
      )
    })
    ...
    it('rejects with 500 errors', async () => {
      ...
      await assert.rejects(
        () => makeRequest({ url: 'http://example.com/500', retries: 0 }),
        'InternalServerError'
      )
    })
  })
  ...
  describe('Request timeouts', () => {
    it('throws when timeout is exceeded', async () => {
      ...
      await assert.rejects(
        () => makeRequest({ url: 'http://example.com/timeout', timeout: 100 }),
        'TimeoutError'
      )
    })
  })
  ...
  describe('Retries', () => {
    it('repeats a request after receiving a non-successful response', async () => {
      ...
      try {
        await makeRequest({ url: 'http://example.com/retries', retries: 3 })
      } catch {
  ...
  describe('Rate limiting', () => {
    it('waits until the rate limit timeout', async () => {
      ...
      const now = Date.now()
      const response = await makeRequest({ url: 'http://example.com/ratelimit' })
